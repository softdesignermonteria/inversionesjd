...
abstract class Tag {
	...
	/**
 	 ...
 	 * @static
 	 */
	static public function textFieldExt($params){
		$numberArguments = func_num_args();
		$params = Utils::getParams(func_get_args(), $numberArguments);
		if(!isset($params[0])) {
			$params[0] = $params['id'];
		...
		$code = "<input type='text' id='".$params[0]."_id'  name='".$params[0]."_id' value='$value' ";
		foreach($params as $_key => $_value){
			if(!is_integer($_key)){
				$code.="$_key='$_value' ";
			}
		...
		$code.=" />\r\n";
		$code.= "<input type='text' id='".$params['campo']."' size='40' ";
		foreach($params as $_key => $_value){
			if(!is_integer($_key)){
				$code.="$_key='$_value' ";
			}
		...
		$code.=" />\r\n";
		$code.= "<button id='buscar_$params[0]' name='buscar_$params[0]' type='button' >Buscar</button>";
		return $code;
	}
	...
	static public function dateFieldMod($name){
		static $i = false;
		$numberArguments = func_num_args();
		$params = Utils::getParams(func_get_args(), $numberArguments);
		...
		if($i == false){
			$i = true;
			$code .= "<script type=\"text/javascript\" src=\"".core::getInstancePath()."public/javascript/jscalendar/calendar.js\"></script>";
			$code .= "<script type=\"text/javascript\" src=\"".core::getInstancePath()."public/javascript/jscalendar/calendar-setup.js\"></script>"; 
			$code .= "<script type=\"text/javascript\" src=\"".core::getInstancePath()."public/javascript/jscalendar/$language.js\"></script>";   
		}
		
		$code .= Tag::textField($params);
		$code .= Tag::image("calendar.gif","id: ".$params['id']."tigger","style: cursor: pointer;")."\n";
			
		
		$code .= Tag::stylesheetLink("style-calendar/$theme");
