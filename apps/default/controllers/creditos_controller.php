	...
	class CreditosController extends ApplicationController {
	 ...
	 public function beforeFilter(){
										 ...
										 $rec->banco               = "";
											...
											if($rec->save()==false){
													...
													foreach($detallesc->find(" creditos_id = '$creditos->id' and anulado = 0 ") as $items): 			
															...
															$detalles_rec->detalle_cxc_id         = $items->id;
															$detalles_rec->recibos_caja_id        = $rec->id;
															$detalles_rec->descripcion            = "Descuento martes ".$items->id."-".$rec->prefijo.$rec->consecutivo."-".$detalles->codigo;
															$detalles_rec->valor                  = $items->valor ;
															$detalles_rec->codigo                 = $items->codigo;
															$detalles_rec->vencimiento            = $items->vencimiento;
															$detalles_rec->descuento              = 0;
															$detalles_rec->dias_intereses         = 0;
															$detalles_rec->capital                = $capitaltmp;
															$detalles_rec->intereses              = $items->valor - $capitaltmp;
		...
		public function showAction($id=''){
			...
			$this->setParamToView("detalles",$this->DetalleCreditos->find("creditos_id = '$id' and anulado = 0"));
			$this->setParamToView("creditos_id",$id);
			$this->setParamToView("fecha",$Creditos->fecha);
			$this->setParamToView("total_credito2",$Creditos->total_credito);
		...
		public function updateAction(){
			...
			//si no hay error de valiaciones o cualquier otra novedad
			$cr = new Creditos();
			$drc = new DetalleRecibosCaja();
			if( $drc->count("anulado = 0 and creditos_id = '". $_REQUEST["id"]."'  ") == 0 ){
				$sw=1;Flash::error("Esta Credito tiene recibos de caja abonados no se puede modificar.. primero anule todos los recibos y luego modifique");
				}
			/*$sol = new Solicitud();
			if( $sol->count("anulado = 0 and id = '".$_REQUEST["solicitud_id"]."' and clientes_id = '".$_REQUEST["clientes_id"]."' ") == 0 ){
				$sw=1;Flash::error("Esta Solicitud No Corresponde a este Cliente");
